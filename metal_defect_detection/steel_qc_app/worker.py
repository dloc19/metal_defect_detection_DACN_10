...
class Worker(QtCore.QThread):
    ...
    def __init__(self, source, model, idx_to_class, save_dir, onnx_inference=None, parent=None):
    ...
    def run(self):
        ...
        if not cap.isOpened():
            ...
            # Emit error state as not OK to avoid green OK styling
            self.frame_ready.emit(dummy, dummy, "Không mở được nguồn video/camera", False)
            return

        # Initialize CAM objects only if using PyTorch model
        cam_top = CamPlusPlus(self.model, CAM_TOP_LAYER) if self.model else None
        cam_aux = CamPlusPlus(self.model, CAM_AUX_LAYER) if self.model else None
        ...
        while not self.stopped:
            ...
            if self.onnx_inference:
                # Use ONNX inference (faster)
                cls_name, max_conf, probs = self.onnx_inference.predict(frame)
            else:
                # Use PyTorch inference
                if not self.model:
                    status, is_ok = "Model not loaded", False
                    continue
            ...
            if max_conf < ALPHA_THR:
                ...
                # For ONNX, skip Grad-CAM (not easily available)
                if self.onnx_inference:
                    status = f"✖ {cls_name} ({max_conf:.2f})"
                    is_ok = False
                else:
                    # Use PyTorch with Grad-CAM
                    def try_cam(camer):
                        cam = camer(x)
